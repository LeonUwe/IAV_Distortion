...
class Minigame_UI:
    ...
    def __init__(self, sio: AsyncServer, environment_mng: EnvironmentManager, behaviour_ctrl: BehaviourController,
        ...
        self._minigame_controller: Minigame_Controller = Minigame_Controller.get_instance(sio=self._sio,
                                                                                          minigame_ui_blueprint=self.minigame_ui_blueprint)
        self._minigame_controller.set_minigame_start_callback(self._minigame_started_callback)
        try:
            self._driving_speed_while_playing = self.config_handler.get_configuration()['minigame'][
                'driving_speed_while_playing']
        except KeyError:
            logger.warning(
                "No valid value for minigame: driving_speed_while_playing in config_file. Using default value of 30")
    ...
    def make_vehicles_drive_continuously(self, *players: str) -> None:
        ...
        for player in players:
            ...
            if vehicle_id is None:
                logger.warning(
                    "Minigame_Manager.make_vehicles_drive_continuously: No vehicle_id was found for vehicle %s of player %s. "
